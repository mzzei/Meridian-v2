...
- Porta local: **3457** (`node serve.js` — `PORT` env sobrescreve). **3456 é do outro projeto** (aviso no topo do `serve.js`: "NUNCA misturar").
  > ⚠️ **Armadilha encontrada e CORRIGIDA em 2026-08-05:** `.claude/launch.json` rodava o `serve.js` **deste** repo mas declarava `"port": 3456` com o nome `"WorldCupAgent"` — porta e nome do projeto do qual o v2 se define como diferente (inv. 1). Como a allowlist do Worker só tem **3457**/8787, preview em 3456 levava **403 em toda rota** (`/v1`, `/af`, `/fd`, `/fpl`) — sintoma que se disfarça de "fonte de dados quebrada". Medido no Worker em produção: `Origin: http://localhost:3456` → **403 sem cabeçalho CORS**; `Origin: http://localhost:3457` → **200 com `Access-Control-Allow-Origin`**. Hoje o `launch.json` declara **3457** nas duas configurações, com `autoPort: false` **de propósito** — porta ocupada deve falhar alto, nunca escorregar para uma porta fora da allowlist. Preview que só exercita módulo local (import + função pura) funciona em qualquer porta; **análise ao vivo exige 3457**.
...
| AF standings/fixtures 2026 | **Bloqueado no Free** (clubes) — contornado pela cascata da PARTE IX (FEITA no 85) |
| Técnico determinístico | **AF free** via search teams+coachs |
| Escalação **provável** | rawFacts da pesquisa > F2 do modelo > geometria inferida — sempre com **badge de proveniência** (inv. 34) |
| Escalação **confirmada** (dia de jogo) | **determinística, sem Anthropic**: `lineup-confirmed.js` com precedência **AF confirmada > ESPN starters**, dentro de `isMatchDayWindow` (FEITO no 87 — não é mais "só web_search") |
| Métricas EPL | **FPL** (Worker) + busca |
| xG outras ligas | **web_search** (estimativa entra nos lambdas com proxy declarado, nunca no campo `xg_*` — inv. do 108) |
| Histórico evento | StatsBomb Open se ano na query |
| Sem Anthropic | Sem análise/chat LLM completo — mas o XI confirmado e o poll de escalação continuam funcionando |

**Paridade com V1: FEITA** (PARTE IX, shell 85 — cascata AF adaptativa + coverNote duro + `_lineups` de F2). Esta seção descreve o estado **corrente**; se divergir do código, o código vence.
...
| **133·docs2** | **2º passe: PARTES I–III, invariantes e auditoria de símbolos** (docs, sem bump). O 1º passe cobriu cabeçalho/mapa; este foi ao corpo técnico. **Conferem contra o código** (não presumido): as 5 competições e seus códigos (§2); porta 3457 no `serve.js`; `routeUserIntent` **linha a linha** — ordem anexos→explicit_full→match_query→default, os 4 gatilhos do `isExplicitFullAnalysisAsk` e os 5 passos do `looksLikeMatchQuery`, incluindo o caso `4-3-3 vs 4-4-2`→chat (§5/§6); `MODEL_PROFILES` budget 0 + searches 1/2/3, `currentModel=claude-sonnet-5`, `MODEL_CTX` 200k/1M/1M, `MODEL_PRICE` sonnet 3/15/2.70, `_noThinkModel=/claude-sonnet-5/`, F1+gaps+verify fixos em Haiku, assert que proíbe `budget>0` (§7.3); allowlist e rotas do Worker batendo com o `/health` em produção (§12); invariante 7 (as 2 ocorrências de `allowed_domains` no código são comentários PROIBINDO o uso) e invariante 33 (os `JSON.parse` restantes vivem DENTRO do `parseAnalysisJson`). **Auditoria de símbolos**: 286 identificadores citados no doc conferidos contra `js/`+`worker/`+`motor/`+`tests/`+`tools/` — **16 ausentes, todos corretos** (hashes de commit, `EFFORT_*`/`setEffort`/`pickEffort` que o doc declara removidos no 70, e `_soEvt`/`_soTecF2` que só aparecem descrevendo o F2_SCHEMA removido no 100). **Corrigidos**: (1) §14 se intitulava "quadro atual" mas descrevia o mundo **pré-shell-87** — dizia escalação confirmada = "web_search, P2 pendente" quando existe caminho DETERMINÍSTICO (`lineup-confirmed.js`, AF confirmada > ESPN starters, zero Anthropic) e dava PARTE IX como "próximo" estando FEITA desde o 85; (2) §1 ganhou a **armadilha de porta**: `.claude/launch.json` roda o `serve.js` deste repo mas declara `port 3456` com nome `WorldCupAgent` — porta e nome do projeto do qual o v2 se define como diferente (inv. 1) — e como a allowlist do Worker só tem 3457, preview em 3456 toma **403 em toda rota**, sintoma que se disfarça de "fonte de dados quebrada" |
