...
- Porta local: **3457** (`node serve.js` — `PORT` env sobrescreve). **3456 é do outro projeto** (aviso no topo do `serve.js`: "NUNCA misturar").
  > ⚠️ **Armadilha real (registrada em 2026-08-05):** `.claude/launch.json` roda o `serve.js` **deste** repo mas declara `"port": 3456` e se chama `"WorldCupAgent"` — o nome e a porta do projeto do qual o v2 se define como diferente (inv. 1). Consequência concreta: a allowlist do Worker só tem **3457**/8787, então um preview em 3456 leva **403 em toda rota** (`/v1`, `/af`, `/fd`, `/fpl`) e a sessão perde tempo achando que é bug de fonte de dados. Preview que só exercita módulo local (import + função pura) funciona em qualquer porta; **preview que faz análise ao vivo tem que estar em 3457**. Corrigir o `launch.json` (porta 3457 + nome do projeto) resolve na raiz.
